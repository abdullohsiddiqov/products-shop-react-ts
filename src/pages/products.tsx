...
export const Products: React.FC = () => {
  ...
  return (
    <div className="container">
      ...
      <input className="form-control" list="datalistOptions" id="exampleDataList" placeholder="Type to search..." value={searchValue} onChange={handleSearchChange}/>
    ...
    products.map((product) => (
      <li key={product.id} className='brdr'>
        <Link to={`/page/products/${product.id}`}>
          <img src={product.thumbnail} alt={`${product.title} thumbnail`} className='prod-img'/>
          <h2>{product.title}</h2>
          <p>Price: ${product.price}</p>
          <p>Rating: {product.rating}</p>
        </Link>
        <li className='end'>
        <button type="button" className="btn-no" onClick={() => handleAddToCart(product)}>Add to Cart</button>
        </li>
      </li>
    ))
  )}
